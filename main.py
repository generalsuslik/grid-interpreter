...
class Interpreter:
    ...
    def __init__(self):
        self.grid = Grid()
        self.commands = [i[:-1].strip() for i in open("program.txt").readlines() if len(i) > 2]
    ...
    def __str__(self):
        return f"{self.executable_commands}"


program = Interpreter()
program.get_procedures()
program.parse(program.commands)
print(program)
